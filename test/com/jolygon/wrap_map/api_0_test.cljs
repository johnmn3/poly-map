...
(deftest wrap-map-arity-test
  ...
  (is (= nil ((wrap :a 1) :b)))
  (is (= "invalid-arity 3"
         (try ((wrap) 1 2 3) (catch :default e
                               (str (name (:error (ex-data e)))
                                    " "
                                    (:arity (ex-data e)))))))
  (is (= "invalid-arity 4"
         (try ((wrap) 1 2 3 4) (catch :default e
                                 (str (name (:error (ex-data e)))
                                      " "
                                      (:arity (ex-data e))))))))
...
(deftest test-wrap-map-impls
  (let [dm (w/with-wrap (wrap :a 1 :b 2)
    ...
    (let [updated-dm (w/dissoc-impl dm :invoke)]
      (is (not (w/contains-impl? updated-dm :invoke)) "Implementation should be removed after dissoc-impl"))))
...
(deftest test-impl-api-persistent
  (let [m0 w/empty-wrap
    ...
    (is (not (w/contains-impl? m0 :-lookup_k)) "Empty map shouldn't contain impl")
    (is (nil? (w/get-impl m0 :g-lookup_ket_k)) "Getting non-existent impl returns nil")

    (let [m1 (w/assoc-impl m0 :-lookup_k f1)]
      (is (w/contains-impl? m1 :-lookup_k) "Should contain impl after assoc-impl")
      (is (= f1 (w/get-impl m1 :-lookup_k)) "Should retrieve correct impl")
      (is (map? (w/get-impls m1)) "get-impls returns a map")
      (is (= f1 (get (w/get-impls m1) :-lookup_k)) "get-impls includes added impl"))

    (let [m2 (w/assoc-impl m0 :-lookup_k f1 :-lookup_k_nf f2)]
      (is (w/contains-impl? m2 :-lookup_k) "Contains first impl")
      (is (w/contains-impl? m2 :-lookup_k_nf) "Contains second impl")
      (is (= f1 (w/get-impl m2 :-lookup_k)))
      (is (= f2 (w/get-impl m2 :-lookup_k_nf))))

    (let [m1 (w/assoc-impl m0 :-lookup_k f1)
          m3 (w/dissoc-impl m1 :-lookup_k)]
      (is (not (w/contains-impl? m3 :-lookup_k)) "Should not contain impl after dissoc-impl"))

    (let [m1 (w/assoc-impl m0 :-lookup_k f1)
          new-impls {:-assoc_k_v f2}
          m4 (w/with-wrap m1 new-impls)]
      (is (not (w/contains-impl? m4 :-lookup_k)) "Old impl gone after with-wrap")
...
(deftest test-override-assoc-validation
  (let [validated-map
    ...
    (let [m1 (assoc validated-map :a "hello")]
      ...
      (is (instance? mi/WrapMap+-assoc_k_v m1)))
...
(deftest test-override-transient-logging
  (let [log (atom [])
        ...
        (w/assoc
          (wrap)
          :T_-assoc!_k_v
          (fn T_-assoc!_k_v [_ t-m k v]
            (swap! log conj [:T_assoc! k v])
            (assoc! t-m k v))
          :T_-dissoc!_k
          (fn T_-dissoc!_k [_ t-m k]
            (swap! log conj [:T_without! k])
            (dissoc! t-m k)))
    ...
    (is (= [[:T_assoc! :a 1]
            [:T_assoc! :b 2]
            [:T_without! :a]
            [:T_assoc! :c 3]] @log) "Log contains correct operations")))
