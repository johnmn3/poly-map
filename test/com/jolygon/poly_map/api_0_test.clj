...
(deftest test-poly-map-impls
  (let [dm (poly/set-impls (poly-map :a 1 :b 2)
                           {::pm/invoke (fn [_env & args] (apply + args))})]

    (is (poly/contains-impl? dm ::pm/invoke) "Should contain ::pm/invoke impl")
    (is (not (poly/contains-impl? dm :non-existent)) "Should not contain :non-existent impl")

    (is (fn? (poly/get-impl dm ::pm/invoke)) "::pm/invoke impl should be a function")
    ...
    (let [impls (poly/get-impls dm)]
      ...
      (is (contains? impls ::pm/invoke) "Implementations should contain ::pm/invoke"))
    ...
    (let [new-impls {::new-impl (fn [] "new")}
      ...
      (is (not (poly/contains-impl? updated-dm ::pm/invoke)) "Should not contain old impl after set-impls"))

    (is (= {:a 1 :b 2} (poly/get-coll dm)) "get-coll should return the underlying collection")

    (let [updated-dm (poly/dissoc-impl dm ::pm/invoke)]
      (is (not (poly/contains-impl? updated-dm ::pm/invoke)) "Implementations should be removed after dissoc-impl"))
...
(deftest test-override-assoc-validation
  (let [validated-map
    ...
    (let [m1 (assoc validated-map :a "hello")]
      ...
      (is (instance? com.jolygon.poly_map.api_0.impl.PolyMap m1)))
